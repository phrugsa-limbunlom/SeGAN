...
if __name__ == "__main__":
    logging.basicConfig(filename='LSTM_model_test.log', level=logging.INFO,
    ...
    for time_horizon in time_horizons:
        for step in look_back:
            ...
            LSTM_model = LSTMModel(input_shape=(X_test.shape[1], X_test.shape[2]), output_shape=1, y_scaler=y_scaler)
            LSTM_model.compile_model()
            LSTM_model.model.load_weights(
                f'../training/weight/{folder}/lstm_model_{step}days_{time_horizon}days_weights.weights.h5')

            # Predict closing price
            predicted_closing_price = LSTM_model.model.predict(X_test)
            ...
            logging.info(f"(LSTM) Predicted Closing Price: {predicted_closing_price}")

            # Evaluate model
            rmse = root_mean_squared_error(real_closing_price, predicted_closing_price)
            logging.info(f'LSTM Model RMSE Testing set: {rmse:.3f}')
            ...
            ax.set(xlabel='Date',
                   ...
                   title='Real vs Predicted Closing Prices (LSTM)')
            ...
            # Show the plot
            plt.savefig(f"../testing/figure/{folder}/lstm_closing_prices_{step}days_{time_horizon}days.png")
            plt.show()
